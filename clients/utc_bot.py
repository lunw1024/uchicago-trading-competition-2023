...
class UTCBot:
    ...
    async def swap(self, name: str, qty: int) -> pb.SwapResponse:
        """
        Perform a swap on the exchange

        Args:
            swap_name (str): The name of the swap to perform
            qty (int): The multiple of the swap to perform

        Returns:
            pb.SwapResponse The response from the exchange
        """

        resp = await self.__service_stub.swap(
            creds=self.creds,
            swap=pb.SwapSpec(name=name, quantity=qty),
        )
    ...
    async def get_positions(self) -> pb.PositionResponse:
        """
        Gets the user's positions

        Returns:
            pb.PositionResponse The response from the exchange
        """
        
        resp = await self.__service_stub.get_positions(
            creds=self.creds,
        )
    ...
    def preprocess_message(self, update: pb.FeedMessage):
        ...
        if msg_type == "generic_msg":
            ...
            if event_type == pb.GenericMessageType.ROUND_ENDED:
                self.positions = {}
        ...
        if msg_type == 'position_msg':
            self.positions = update.position_msg.positions

        if msg_type == 'asset_expiry_msg':
            assets = update.asset_expiry_msg.assets
            for expiring_asset in assets:
                if expiring_asset.asset not in self.positions:
                    continue
                if expiring_asset.underlying not in self.positions:
                    self.positions[expiring_asset.underlying] = 0
                self.positions[expiring_asset.underlying] += self.positions[expiring_asset.asset] * expiring_asset.lot_size
                self.positions[expiring_asset.asset] = 0
